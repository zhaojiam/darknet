...
void cuda_set_device(int n) try {
    gpu_index = n;
    /*
    DPCT1093:125: The "n" device may be not the one intended for use. Adjust the
...
void check_error(dpct::err0 status)
{
    ...
    */
    dpct::err0 status2 = 0;
    /*
    DPCT1000:127: Error handling if-stmt was detected but could not be
    rewritten.
    */
    if (status != 0)
    {
        /*
        DPCT1009:131: SYCL uses exceptions to report errors and does not use the
        ...
        */
        ...
        printf("CUDA Error: %s\n", s);
        /*
        DPCT1001:126: The statement could not be removed.
        */
        assert(0);
    ...
    /*
    DPCT1000:129: Error handling if-stmt was detected but could not be
    rewritten.
    */
    if (status2 != 0)
    {
        /*
        DPCT1009:132: SYCL uses exceptions to report errors and does not use the
        error codes. The call was replaced by a placeholder string. You need to
        rewrite this code.
        */
        const char *s = "<Placeholder string>";
        ...
        /*
        DPCT1001:128: The statement could not be removed.
        */
        assert(0);
...
dpct::dnnl::engine_ext cudnn_handle()
{
    ...
    static dpct::dnnl::engine_ext handle[16];
    int i = cuda_get_device();
    if(!init[i]) {
        handle[i].create_engine();
        init[i] = 1;
    }
    return handle[i];
}
